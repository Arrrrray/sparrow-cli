...
//修改gio版本号
function changeGioVersion(filepath, gioConf, newConf) {
  return new Promise((resolve, reject) => {
    fs.writeFile(filepath, jsonFormat({ ...gioConf, ...newConf }), err => {
      if (err) {
        Log.error(err);
        process.exit(1);
      } else {
...
// //修改gio版本号
// function changeGioJsVersion(filepath, gioConf, newConf) {
...
module.exports = async function () {
  ...
  // 版本配置文件路径    
  const gioConfPath = `${Config.dir_root}/utils/growingio/gioConfig.json`;

  // 版本配置文件路径    
  // const gioJsConfPath = `${Config.dir_root}/utils/growingio/gioConfig.js`;
  // console.log('gioJsConfPath', gioJsConfPath)
  // package.json文件路径    
  ...
  //  获取gio配置
  const gioConf = require(gioConfPath);

  //  获取gio配置
  // const gioJsConf = require(gioJsConfPath);
  // console.log('gioJsConf', gioJsConf)
  ...
  switchFunc(answer.isRelease).then(() => {
    ...
    // 修改本地版本文件 (当为发行版时)
    rewriteLocalVersionFile(versionConfPath, versionConf).then(res => {
      if (res === 2) {
    ...
    // 修改gio版本号文件 (当为发行版时)
    changeGioVersion(gioConfPath, gioConf, versionConf).then(res => {
      if (res === 2) {
        Log.success('gioConfig.json文件修改成功！！！');
      }
    });

    // 修改本地package.json文件 (当为发行版时)
    rewritePackageConfigFile(packageConfPath, packageConf, versionConf).then(res => {
      if (res === 2) {
